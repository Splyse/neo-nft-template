...
# Note: I haven't found any documentation on best practice for when one should use
Runtime.Log vs Runtime.Notify, so I am using Log for recording changes on the blockchain/events
(transfers, approvals, minting, and config changes) and Notify for everything else.
"""
...
from boa.interop.Neo.Runtime import GetTrigger, CheckWitness, Log, Notify, Serialize
...
def Main(operation, args):
    ...
    - mintToken(properties, URI, owner, extra_arg): create a new NFT token with the specified
        properties and URI
    ...
    - tokensOfOwner(owner, starting_index): returns a list that contains less than or equal to ten
        of the tokens owned by the specified address starting at the specified index.
    ...
    - uri(token_id): Returns a distinct Uniform Resource Identifier (URI) for a given asset.
        The URI data of a token supplies a reference to get more information about a specific token
        or its data.

    setters
    - setName(name): sets the name of the token
    - setPostMintContract(contract_address): sets the contract freshly minted tokens get sent to
        by default
    ...
    # Note: for some operations, it appears as though there has been a change in neo-boa where
    # integers are no longer decoded to bytes. So, some methods may not work if you pass in an
    # integer for token_id. If so, convert the integer to a bytes object before passing it.
    Example:
        >>> x = 1349
...
def do_mint_token(ctx, args):
    ...
    :param list args:
        ...
        2: byte[] t_owner (optional): default is postMintContract, can be a user address,
    ...
    # the int 0 is represented as b'' in neo-boa, this caused bugs throughout my code
    # hence the reason why token id's start at 1 instead
    t_id += 1
    ...
    # if nft contract owner didn't pass an address, transfer the newly minted token to the default
    # contract. if nft contract owner did pass an address and it is a smart contract, transfer
    # the newly minted token to the passed contract
    this_contract = GetExecutingScriptHash()
    if len(t_owner) != 20:
        t_owner = Get(ctx, 'postMintContract')
        if GetIsPayable(t_owner):
            ...
            Notify('Recipient contract is not payable')
            return False
    ...
    Log(concat('total supply updated: ', t_id))
    return t_id
...
def do_tokens_of_owner(ctx, t_owner, start_index):
    """This method returns ten of the owner's tokens starting at the given index. The index is used
    for paginating through the results. The index is needed for the situation where the owner's
    list of tokens could be quite large.

    For example, the specified owner could have 100,000 tokens out of 1,000,000 minted tokens.
    In such a scenario, returning the full list of token id's would be quite expensive and could
    possibly be too large to return anyway. Hence, @hal0x2328 recognized the need to paginate the
    data in such a scenario. So, if we know that this user has a balanceOf() 100,000 tokens and
    we want to get their 10 most recent tokens, then our call would be like so:
    `testinvoke {my_hash} tokensOfOwner [{owner address string}, 999990]`
    The results would look something like:
        [{'type': 'ByteArray',
        'value': '800a00010100010200010300010400010500010600010700010800010900010a'}]
    ...
    if len(t_owner) == 20:
        ...
        count = 0  # counter to make sure that the returned tokens string doesn't get too big
        token_list = []
        token_iter = Find(ctx, t_owner)
        # while loop explained: keep looping through the owner's list of tokens until 10 have been
        # found beginning at the starting index.
        # if statement explained: once a key has been found matching my search key
        # (or of greater value), append the token id to the list, increment the counter,
        ...
        while (token_iter.IterNext()) and (count < 10):
            if (token_iter.IterKey() >= start_key) or (count > 0):
                token_list.append(token_iter.IterValue())
                count += 1
...
def do_transfer_from(ctx, args):
    ...
    # len(t_owner) == 20 and len(t_receiver) == 20, thus the length of authorized_spender should
    # be 40
    ...
    if authorized_spend == concat(t_from, t_to):
        # 1. Is t_to a smart contract? 2. Is t_to payable?
        # If True for both, invoke the transfer_to_smart_contract method,
        # if transfer_to_smart_contract() returns False, then reject the transfer
        if GetContract(t_to):
            if GetIsPayable(t_to):
...
def do_transfer(ctx, caller, args):
    ...
    # Verifies that the calling contract has verified the required script hashes of the
    # transaction/block
    is_token_owner = CheckWitness(t_owner)
    ...
    if is_token_owner:
        ...
        # always only transferring one NFT at a time, backwards compatibility
...
def remove_token_from_owners_list(ctx, t_owner, t_id):
    ...
    # if Delete returns True, that means the token was successfully deleted and
    # we should decrement the owner's balance.
    # otherwise, the token didn't exist/didn't belong to the owner, so Delete returns False in that
    # case.
...
def add_token_to_owners_list(ctx, t_owner, t_id):
    ...
    :param StorageContext ctx: current store context
    :param byte[] t_owner: token owner (could be either a smart contract or a wallet address)
    :param bytes t_id: token ID
    :return: token id
    :rtype: integer
    """
...
def transfer_to_smart_contract(ctx, args, is_mint):
    ...
    :param StorageContext ctx: current store context
    :param list args:
        0: byte[] transfer from address (who is sending the NFT)
        1: byte[] transfer to address (who is receiving the NFT)
        2: bytes token id
    ...
    # the `is_mint` check is needed because you can't get the token owner for a token that hasn't
    # finished being minted yet
    if is_mint is False:
        t_owner = Get(ctx, t_id)
        if t_owner != t_from:
